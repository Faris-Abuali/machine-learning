...
def euclidean_distance(a, b):
    ...
    # first term produces a column vector (mx1) with the sum of the squares of each row of a
    # second term produces a matrix (mxn) with the dot product of each vector of `a` with each vector of `b`
    # third term produces a row vector (1xn) with the sum of the squares of each row of b
...
class KNN(BaseEstimator, ClassifierMixin):
    ...
    def predict(self, X):
        ...
        # Generate an empty array of shape (X.shape[0],) with the same data type as self.train_points[1]
        ...
        y_res = np.empty(num_data_points, dtype=y.dtype)
...
# np._c is a short for short for "column stack" which stacks 1D arrays as columns into a 2D array.
# Purpose of np.c_:
